...
--
-- The database was still carrying the v1 goods-auction shape. Three tables had
-- the wrong columns -- listings had seller_id/start_price/ends_at, shipments had
-- user_id/package_weight, payments had amount/application_fee_amount -- so every
-- query against them died on "column does not exist". That is why deliveries,
-- the award queue and the admin listings/shipments/payments screens all 500ed.
--
-- Four enum TYPES had drifted too, which a column-level comparison does not
-- show: listing_status was still (active, sold, ended, cancelled) and
-- payment_status still (pending, succeeded, failed, refunded). Rebuilding the
-- tables alone fails on "invalid input value for enum listing_status: draft".
--
-- reviews and shipment_events are rebuilt only because they own columns of the
-- drifted enums. reviews is empty, and the seven shipment events belong to the
-- two v1 shipments going away.
--
-- Rebuilt rather than altered: a v1 row carries no pickup window, budget or
-- origin, so there is nothing in it that maps onto a transport job.
--
-- Untouched: user, user_roles, offers, carriers, vehicles, carrier_documents,
-- conversations, messages and expedion_quotes. The imported quotes are safe in
-- particular -- listing_id is null on every one, so none references a listing.

-- 1. Remove the v1 tables the schema no longer defines.
...
-- 2. Clear references that would dangle once the tables are rebuilt. The
-- conversations and their messages are kept; only the stale link goes.
UPDATE "conversations" SET "listing_id" = NULL WHERE "listing_id" IS NOT NULL;--> statement-breakpoint
-- 3. Remove the drifted tables, children first. CASCADE also removes the
-- inbound foreign keys, which step 6 restores.
DROP TABLE IF EXISTS "shipment_events" CASCADE;--> statement-breakpoint
...
-- 4. Rebuild the drifted enum types, now that nothing depends on them.
DROP TYPE IF EXISTS "public"."listing_status" CASCADE;--> statement-breakpoint
...
DROP TYPE IF EXISTS "public"."actor_role" CASCADE;--> statement-breakpoint
CREATE TYPE "public"."actor_role" AS ENUM('system', 'shipper', 'carrier', 'driver', 'operator', 'admin');--> statement-breakpoint
DROP TYPE IF EXISTS "public"."review_role" CASCADE;--> statement-breakpoint
CREATE TYPE "public"."review_role" AS ENUM('shipper', 'carrier');--> statement-breakpoint
-- 5. Canonical definitions, taken verbatim from 0000_old_slayback.
...
-- 6. Foreign keys owned by the rebuilt tables.
...
-- 7. Foreign keys from other tables back into them.

...
export function CreateAddressForm() {
  ...
  const [viewState, setViewState] = useState({
    longitude: 2.3522,
    latitude: 48.8566,
    zoom: 6, // Zoomed out to show France
  });
  ...
  // Also validates that location is in France
  const handleReverseGeocode = useCallback(
    async (lng: number, lat: number): Promise<boolean> => {
      ...
      try {
        ...
        if (result) {
          // Validate that the location is in France
  ...
  const handleMapClick = useCallback(
    async (event: { lngLat: { lng: number; lat: number } }) => {
      ...
      // Set marker temporarily while we validate
      setMarker({ lng, lat });

      // Reverse geocode validates France and fills form
      // If not in France, reverseGeocode will set error and clear marker
  ...
  return (
    <div className="w-full mx-auto">
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">{t("addAddress")}</h1>
      </div>
            ...
            {marker && (
              <Marker
                ...
                onDragEnd={async (e) => {
                  ...
                  setMarker({ lng, lat });
                  // reverseGeocode validates France and sets error if outside
                  await handleReverseGeocode(lng, lat);
                }}
              >
                <MapPin
